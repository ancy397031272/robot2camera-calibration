...
if(verbose)
    fprintf('Saving chessboard real and guess images\n');
end

[~, projectedGuess, projectedInital] = ProjectError(points, cameraParams, worldPoints, armPose, inliers, inital);
[~, projectedSolution, projectedXYZSol] = ProjectError(points, cameraParams, worldPoints, armPose, inliers, solution);
...
for i = 1:size(projectedGuess,3)
   ...
   legend([p1, p2, p3, p4, p5, p6, p7, p8, p9], 'detected', 'guess', 'solution','base initial guess','tcp initial guess','grid initial guess', 'base solution', 'tcp solution', 'grid solution')
   annotation('textbox', [0,0,.10,.10], 'String', 'Red is x, Green is y, Blue is z', 'FitBoxToText','on','BackgroundColor','white','Color','red')
   hold off
   saveas(f, sprintf('chessboards/chessboard%i.png',i))
end
if(verbose)
    fprintf('Done saving chessboard real and guess images\n');
end
